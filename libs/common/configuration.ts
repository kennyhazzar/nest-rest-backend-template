...
/**
 * Shared by every app (apps/backend, apps/auth-service, ...): same YAML file, same
 * `${VAR:-default}` / `${VAR:?err}` env-substitution syntax, same `CONFIG_FILE` env override.
 * Each app just passes this into its own `ConfigModule.forRoot({ load: [loadConfiguration] })`.
 */
